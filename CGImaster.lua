...
local window = Window:new(-1, -1, 350, 237)
...
local disclaimer2 = Label:new(10, 105,tpt.textwidth('- Not all images will render properly. But the accuracy is about 95%.'),12, '- Not all images will render properly. But the accuracy is about 95%.')
local disclaimer3 = Label:new(10, 117,tpt.textwidth('- The image name is a name of an image (without .bmp extension)'),12, '- The image name is a name of an image (without .bmp extension)')
local disclaimer4 = Label:new(10, 129,tpt.textwidth('located in CGImaster folder, next to CGImaster.lua script.'),12, 'located in CGImaster folder, next to CGImaster.lua script.')
local disclaimer5 = Label:new(-9, 142,tpt.textwidth('- 1 pixel of the image equals 1 pixel in the game.'),12, '- 1 pixel of image equals 1 pixel in game.')
local disclaimer6 = Label:new(10,154,tpt.textwidth('- The file format has to be 24 bit BMP.'),12, '- The file format has to be 24 bit BMP.')
local disclaimer7 = Label:new(10,167,tpt.textwidth('- You can convert JPG, PNG and other formats to 24 bit BMP using:'),12, '- You can convert JPG, PNG and other formats to 24 bit BMP using:')
local disclaimer8 = Label:new(10,179,tpt.textwidth('online-converting.com/image/convert2bmp'),12, 'online-converting.com/image/convert2bmp')
local disclaimer9 = Label:new(10,191,tpt.textwidth('AND THE MOST IMPORTANT:'),15, 'AND THE MOST IMPORTANT:')
local disclaimer10 = Label:new(10,206,tpt.textwidth('DO NOT post any saves containing deco generated by this script,'),12, 'DO NOT post any saves containing deco generated by this script,')
local disclaimer11 = Label:new(10,218,tpt.textwidth('because it is against TPT rules.'),12, 'because it is against TPT rules.')
local copyButt = Button:new(tpt.textwidth('online-converting.com/image/convert2bmp')+15,179,30,11)
copyButt:text('COPY')
...
window:addComponent(disclaimer11)
